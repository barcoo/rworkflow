class Flow
  ...
  def get_counters
    ...
    if counters.present?
      counters = begin
        ...
        CimLog.error("Error loading stored flow counters: #{e.message}")
        nil
      end
    end
  ...
  def get_counters!
    ...
    results = RedisSingleton.get_instance.multi do
    ...
    if list.present?
      ...
      if !raw_objects.empty?
        ...
        if failed.present?
          ...
          CimLog.error("Failed to parse #{failed.size} in workflow #{self.id} for fetcher id #{fetcher_id} at state #{state_name}")
        end
  ...
  def get_state_list(state_name)
    ...
    if state.present?
      ...
      CimLog.error("Tried accessing invalid state #{state_name} for workflow #{id}")
    end
    return list
  end
  ...
  def transition(from_state, name, objects)
    ...
    to_state = begin
      ...
      CimLog.error("Error transitioning: #{e}")
      nil
    end
  ...
  class << self
    ...
    def read_flow_class(id)
      ...
      klass = begin
        ...
        CimLog.report(REPORT_KEY_UNKNOWN_FLOW_CLASS, "Unknown flow class for workflow id #{id}")
        nil
